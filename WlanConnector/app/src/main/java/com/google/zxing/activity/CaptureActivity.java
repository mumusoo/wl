...
//		});
    ...
    public void handleDecode(Result result, Bitmap barcode) {
        ...
        if (TextUtils.isEmpty(resultString)) {
            ...
            Log.i("sscan_data=",resultString);
            // 不能使用Intent传递大于40kb的bitmap，可以使用一个单例对象存储这个bitmap
//            bundle.putParcelable("bitmap", barcode);
//            Logger.d("saomiao",resultString);
                 //   wac.connect("mumusoo","mumusoo1995",WifiConnector.WifiCipherType.WIFICIPHER_WPA);
            resultIntent.putExtras(bundle);
    ...
    private void initBeepSound() {
        if (playBeep && mediaPlayer == null) {
            // The volume off STREAM_SYSTEM is not adjustable, and users found it
            // too loud,
            // so we now play off the music stream.
